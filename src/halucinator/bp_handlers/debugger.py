...
def check_debug_bp(pc: int) -> bool:
    return pc in list(intercepts.debugging_bps.values())
...
class Debugger(object):
    ...
    def __init__(
        ...
        target: QemuTarget,
        avatar: Avatar,
        stack_trace: Optional[StackTraceParser],
    ) -> None:
        self.target = target
        self.avatar = avatar
    ...
    def stop(self) -> bool:
        """
        This function will stop the execution of the target if it is running,
        returning True on success. Returns False and logs an error if called
        while target state is not running.
        """
        if self.get_state() == DebugState.STOPPED:
            log.error("Attempted Stop in STOPPED State")
            return False
        act: Literal[RequestType.STOP] = RequestType.STOP
        resp: Queue[bool] = Queue()
        request = (act, resp)
        self.request_queue.put(request)
        if resp.get():
            return True
