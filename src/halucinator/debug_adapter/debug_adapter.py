...
class LineTranslator(object):
    ...
    the GTIRB view file that displays the disassembled contents to the user,
    and the actual addresses of instructions in the running program.
    ...
    def __init__(self, lineOffsetList: List[Tuple[int, int]]):
        ...
        Parameters:
            lineOffsetList: A sorted list of (line_number, offset) pairs.
        """
        # Because the bisect module in Python < 3.10 does not support key
        # functions, the easiest (and less memory intensive) approach to allow
        # lookups from either line or address is to manage them as two separate
        # but matching lists.
        self.lineList = [r[0] for r in lineOffsetList]
        self.offsetList = [r[1] for r in lineOffsetList]

        global line_translator
        line_translator = self

        # Verify that the input list has both line numbers and offsets in order
        for i in range(len(lineOffsetList) - 1):
            line0, off0 = lineOffsetList[i]
            line1, off1 = lineOffsetList[i + 1]
            if line0 > line1:
                raise ValueError(
                    "LineTranslator list out of order: line %d listed before line %d"
                    % (line0, line1)
                )
            if off0 > off1:
                raise ValueError(
                    "LineTranslator offsets out of order: line %d: %#x is before %#x"
                    % (line0, off0, off1)
                )
...
class HalRuntime(object):
    ...
    def _state_callback(self, state: CallbackState) -> None:
        if state in [CallbackState.STOP, CallbackState.HAL_STOP]:
            self._event_callback("stopped", {"reason": "pause", "threadId": 1})
        elif state in [
            CallbackState.STEP,
            CallbackState.NEXT,
            CallbackState.FINISH,
        ]:
            self._event_callback("stopped", {"reason": "step", "threadId": 1})
    ...
    def _actual_launch(self) -> None:
        ...
        debug_state = self.debugger.get_state()
        if debug_state == DebugState.STOPPED:
...
class DAPServer(object):
    ...
    def __init__(self, debug: Debugger, port: int = DEFAULT_PORT) -> None:
        """ Constructor
