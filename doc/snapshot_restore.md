...
process (fuzzing checkpoints, boot-once-experiment-forever), or days later in
...
* **Fuzzing resume.** A fuzz harness restores a known-good checkpoint before
...
checkpoints, wrong for per-input fuzzing restores.
...
# In-memory checkpoint (fast path — fuzzing / experiment loops):
