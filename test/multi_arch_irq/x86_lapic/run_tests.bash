...
# "IRQ 32 FIRED". Test passes iff both READY and "IRQ 32 FIRED" appear.
set -e
set -x

pkill -9 -f x86_lapic_irq_test 2>/dev/null || true
pkill -9 -f halucinator 2>/dev/null || true
pkill -9 -f trigger_interrupt 2>/dev/null || true
    pkill -9 -f qemu-system-i386 2>/dev/null || true
sleep 2

rm -f ./hal_out.txt
rm -rf tmp/x86_lapic_irq_test

PYTHONUNBUFFERED=1 bash ./test/multi_arch_irq/x86_lapic/run.sh </dev/null \
    > hal_out.txt 2>&1 &
HAL_PID=$!
...
hal_dev_irq_trigger -i 32 || true
sleep 2

TIMEOUT=60; ELAPSED=0; SUCCESS=0
while [ $ELAPSED -lt $TIMEOUT ]; do
    if grep -q "IRQ 32 FIRED" hal_out.txt 2>/dev/null; then SUCCESS=1; break; fi
...
if [ "$SUCCESS" = "1" ]; then
    echo "X86 i386 LAPIC IRQ test PASSED"; exit 0
fi
echo "FAIL: did not see IRQ 32 FIRED within ${TIMEOUT}s of injection"
echo "=== QEMU debug log (exceptions) ==="; tail -40 /tmp/x86_qemu_dbg.log 2>/dev/null; echo "=== hal_out.txt ==="; tail -30 hal_out.txt; exit 1
