...
cd "$SCRIPT_DIR"
SHARED="../p2im-drone/build/p2im-real_firmware/binary/Reflow_Oven"
if [[ -f "$SHARED" ]]; then SRC_ELF="$SHARED"; else
  ...
  SRC_ELF="build/p2im-real_firmware/binary/Reflow_Oven"; fi
cp -f "$SRC_ELF" Reflow.elf
python3 - Reflow.elf Reflow.bin <<'EOF'
import sys,struct
...
EOF
hal_make_addr -b Reflow.elf -o reflow_addrs.yaml
