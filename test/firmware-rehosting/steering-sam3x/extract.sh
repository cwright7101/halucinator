...
cd "$SCRIPT_DIR"
SHARED="../p2im-drone/build/p2im-real_firmware/binary/Steering_Control"
if [[ -f "$SHARED" ]]; then SRC_ELF="$SHARED"; else
  ...
  SRC_ELF="build/p2im-real_firmware/binary/Steering_Control"; fi
cp -f "$SRC_ELF" Steering.elf
python3 - Steering.elf Steering.bin <<'EOF'
import sys,struct
...
EOF
hal_make_addr -b Steering.elf -o steering_addrs.yaml
