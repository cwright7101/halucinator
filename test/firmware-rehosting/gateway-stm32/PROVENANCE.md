# Provenance — test/firmware-rehosting/gateway-stm32/

Auto-modeling rehost of the P2IM **Gateway** firmware (STM32F4 (Cortex-M4)): Firmata gateway (Firmata loop).
...
non-MMIO clock/delay setup is stubbed (see gateway_auto_config.yaml).

**Binary not shipped.** `extract.sh` regenerates Gateway.bin / gateway_addrs.yaml
...
(`binary/Gateway`). The P2IM binaries are AFL-instrumented (an `aflCall`
