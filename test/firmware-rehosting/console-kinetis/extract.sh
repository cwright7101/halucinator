...
cd "$SCRIPT_DIR"
SHARED="../p2im-drone/build/p2im-real_firmware/binary/Console"
if [[ -f "$SHARED" ]]; then SRC_ELF="$SHARED"; else
  ...
  SRC_ELF="build/p2im-real_firmware/binary/Console"; fi
cp -f "$SRC_ELF" Console.elf
python3 - Console.elf Console.bin <<'EOF'
import sys,struct
...
EOF
hal_make_addr -b Console.elf -o console_addrs.yaml
