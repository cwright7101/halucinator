# Provenance — test/firmware-rehosting/console-kinetis/

Auto-modeling rehost of the P2IM **Console** firmware (NXP Kinetis K-series (Cortex-M4)): serial console firmware.
...
non-MMIO clock/delay setup is stubbed (see console_auto_config.yaml).

**Binary not shipped.** `extract.sh` regenerates Console.bin / console_addrs.yaml
...
(`binary/Console`). The P2IM binaries are AFL-instrumented (an `aflCall`
