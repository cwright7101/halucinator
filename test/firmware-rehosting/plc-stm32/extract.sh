...
cd "$SCRIPT_DIR"
SHARED="../p2im-drone/build/p2im-real_firmware/binary/PLC"
if [[ -f "$SHARED" ]]; then SRC_ELF="$SHARED"; else
  ...
  SRC_ELF="build/p2im-real_firmware/binary/PLC"; fi
cp -f "$SRC_ELF" PLC.elf
python3 - PLC.elf PLC.bin <<'EOF'
import sys,struct
...
EOF
hal_make_addr -b PLC.elf -o plc_addrs.yaml
