#!/usr/bin/env bash
# E2E test: the auto-modeling stack boots Heat_Press and runs its real firmware
# logic. heat-press controller (Modbus + control loop). We assert the firmware boots and executes (the PC sampler
...
pkill -9 -f "halucinator.*-n heat_auto" 2>/dev/null || true
...
if grep -q "Letting Unicorn Run" hal_auto_out.txt \
   ...
   && ! grep -qE "Traceback|UcError" hal_auto_out.txt; then
    echo "Heat_Press auto-modeling e2e test PASSED - booted and running firmware logic"
    grep -A4 "PC sample" hal_auto_out.txt | grep -oE "0x[0-9a-f]{8}  x[0-9]+" | head -3 | sed 's/^/    pc /'
    exit 0
else
    echo "Heat_Press auto-modeling e2e test FAILED"
