...
cd "$SCRIPT_DIR"
SHARED="../p2im-drone/build/p2im-real_firmware/binary/Heat_Press"
if [[ -f "$SHARED" ]]; then SRC_ELF="$SHARED"; else
  ...
  SRC_ELF="build/p2im-real_firmware/binary/Heat_Press"; fi
cp -f "$SRC_ELF" Heat.elf
python3 - Heat.elf Heat.bin <<'EOF'
import sys,struct
...
EOF
hal_make_addr -b Heat.elf -o heat_addrs.yaml
