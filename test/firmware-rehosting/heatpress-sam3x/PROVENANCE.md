# Provenance — test/firmware-rehosting/heatpress-sam3x/

Auto-modeling rehost of the P2IM **Heat_Press** firmware (Atmel SAM3X / Arduino Due (Cortex-M3)): heat-press controller (Modbus + control loop).
...
non-MMIO clock/delay setup is stubbed (see heat_auto_config.yaml).

**Binary not shipped.** `extract.sh` regenerates Heat.bin / heat_addrs.yaml
...
(`binary/Heat_Press`). The P2IM binaries are AFL-instrumented (an `aflCall`
