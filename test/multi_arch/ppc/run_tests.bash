...
for ATTEMPT in $(seq 1 $MAX_ATTEMPTS); do
    ...
    rm -f ./hal_out.txt
    PYTHONUNBUFFERED=1 HALUCINATOR_QEMU_PPC="${HALUCINATOR_QEMU_PPC}" \
      ./test/multi_arch/ppc/run.sh </dev/null >hal_out.txt 2>&1 &
    HAL_PID=$!
...
if ! timeout 3m bash -c check_output; then
    ...
    kill $HAL_PID 2>/dev/null || true
    pkill -f qemu-system-ppc 2>/dev/null || true
    exit 1
fi
